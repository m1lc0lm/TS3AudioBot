...
namespace NextGenAudio.Localization
{
	public class LocalizationManager
	{
		...
		static LocalizationManager()
		{
			...
			dynResMan = new DynamicResourceManager("NextGenAudio.Localization.strings", typeof(strings).Assembly);
		...
		private static async Task<E<string>> LoadLanguageAssembly(LanguageData languageDataInfo, CultureInfo culture, bool forceDownload)
		{
			...
			foreach (var currentResolveCulture in GetWithFallbackCultures(culture))
			{
				...
				try
				{
					...
					var resStream = asm.GetManifestResourceStream($"NextGenAudio.Localization.strings.{currentResolveCulture.Name}.resources") ?? throw new NullReferenceException("No stream found");
		...
		private static FileInfo GetCultureFileInfo(CultureInfo culture)
			=> new FileInfo(Path.Combine(Directory.GetCurrentDirectory(), culture.Name, "NextGenAudio.resources.dll"));
