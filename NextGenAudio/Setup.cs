...
namespace NextGenAudio
{
	internal static class Setup
	{
		...
		public static string YoutubeDLPath = "/usr/local/bin/youtube-dl";
		...
		public static int Main(string[] args)
		{
			...
			if (setup is null)
			{
				...
				{
					h.Heading = $"NextGenAudio {SystemData.AssemblyData}";
					h.Copyright = "";
			...
			if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux)){
				...
				{
					Console.WriteLine("Lade Youtube-DL Runter...");
					web.DownloadFileTaskAsync("https://yt-dl.org/downloads/latest/youtube-dl", YoutubeDLPath).ContinueWith(task =>
					{
						if(task.IsCompletedSuccessfully)
						{
							Console.WriteLine("Youtube-DL wurde runtergeladen!");
							Console.WriteLine("Neustart....");
							Thread.Sleep(1000);
							System.Environment.Exit(0);
							Process.Start("cmd /C Start NextGenAudio.exe");
						}
					});
				}
			} else if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
			{
				if (!File.Exists("C:/Users/binfl/Desktop/NextGen-AudioBot/NextGenAudio/bin/Release/netcoreapp3.1/youtube-dl.exe"))
				{
					Console.WriteLine("Lade Youtube-DL Runter...");
					web.DownloadFileTaskAsync("https://yt-dl.org/downloads/2021.06.06/youtube-dl.exe", "C:/Users/binfl/Desktop/NextGen-AudioBot/NextGenAudio/bin/Release/netcoreapp3.1/youtube-dl.exe").ContinueWith(task =>
					{
						if (task.IsCompletedSuccessfully)
						{
							Console.WriteLine("Youtube-DL wurde runtergeladen!");
		...
		public static void SetupLog()
		{
			if (LogManager.Configuration is null)
			{
				...
				{
					using var configStream = Util.GetEmbeddedFile("NextGenAudio.Resources.NLog.config")!;
	...
	{
		[Option('c', "config", Default = null, HelpText = "Specify the path to the NextGenAudio.toml config file.")]
